...
class Grounding(object):
    def __init__(self, network: LiftedBaysianNetwork, flag: bool, basic: bool):
    ...
    def get_evi_variable_card_dict(self):
        node_grounding_dict = self.get_node_grounding_dict()
        evi_grounding_dict = self.get_evi_grounding_dict() if not self.basic else self.get_basic_evi_grounding_dict()
        network = self.abstract_network
        variable_card_dict = {}
    ...
    def build_LBN(self):
        ...
        basic_node_evi_dict = self.get_basic_evi_grounding_dict()
        ...
        if self.lifting_flag:
            ...
            if self.basic:
                value_dict = self.get_basic_node_value_dict()
        ...
        lbn_model = BayesianNetwork(self.get_edges() if not self.basic else self.get_basic_edges())


        for node in network.get_nodes():
            evi_grounding_dict = node_evi_grounding_dict[node.get_name()] if  not self.basic else basic_node_evi_dict[node.get_name()]
            evi_variable_card = evi_variable_card_dict[node.get_name()]
            grounding_value_dict = value_dict[node.get_name()]
            for n in node_grounding_dict[node.get_name()]:
                lbn_model.add_node(n)
                # print(grounding_value_dict[n])
                    ...
                    if n in self.lifting_queue:
                        ...
                        lbn_model.add_cpds(
                            TabularCPD(
                                variable=n,
                                variable_card=2,
                                values=grounding_value_dict[n], state_names={n: ['True', 'False']}))
                else:
                    if n in self.lifting_queue:
    ...
    def get_basic_node_value_dict(self):
        network = self.abstract_network
        node_grounding_list = self.get_node_grounding_dict()
        evi_grounding_list = self.get_evi_grounding_dict()
        value_dict = {}
        for node in network.get_nodes():
            ...
            if not network.get_evidences()[node.get_name()]:
                ...
                for n in node_grounding_list[node.get_name()]:
                    # print(rf'grounding node :{n}')
                    evi_name_list = evi_grounding_list[node.get_name()][n]
                    basic_evi_name_list = self.get_basic_evi_grounding_dict()[node.get_name()][n]
                    grounding_value_list = []
                    variable_dict = {}
                    # evinode variabel dict
                    for evi_n in basic_evi_name_list:
                        if evi_n in self.lifting_queue:
                            lifting_node = network.get_node_from_name(evi_n)
                            # todo currently only consider the single para for lifting node
                            if len(lifting_node.get_domain()) == 1:
    ...
    def get_basic_evi_grounding_dict(self)->dict:
        network = self.abstract_network
        nodes = network.get_nodes()
        res = {}
        for node in nodes:
            grounding_node = generate_n_nodenames_as_list(node)
            basic_evi = {}
            for gn in grounding_node:
                list = []
                if network.get_evidences()[node.get_name()]:
                    for evi_name in network.get_evidences()[node.get_name()]:
                         list.extend(generate_n_nodenames_as_list(network.get_node_from_name(evi_name)))
                basic_evi[gn] = list
            res[node.get_name()] = basic_evi
        return res
...
def calculate_probability(prob_list):
    ...
    for i in range(n):
        dp1 = np.zeros(n + 1)
        for j in range(n):
            dp1[j] += dp[j] * (1 - prob_list[i])  # current node is False
            dp1[j + 1] += dp[j] * prob_list[i]  # current node is True
        dp = dp1
    return dp
...
if __name__ == "__main__":
    ...
    # print(infer.query(["AllPeopleRemoteWorking", "IsShutDown","IsDiagnosed_p1", "IsDiagnosed_p2", "Attends_p1_w1","Attends_p1_w2","Attends_p2_w1","Attends_p2_w2" ]))
    ...
    #
    #     "IsDiagnosed_p1", "IsDiagnosed_p2", "Attends_p1_w1", "Attends_p1_w2",
    #      "Attends_p2_w1", "Attends_p2_w2"]))

    endtime = datetime.datetime.now()
    print(endtime - starttime)
    # print(infer.query(["Drive"]))
